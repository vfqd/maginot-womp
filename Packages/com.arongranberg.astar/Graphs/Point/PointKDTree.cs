	...
	public class PointKDTree {
		...
		void Build (int index, List<GraphNode> nodes, int start, int end) {
			...
			if (end - start <= LeafSize) {
				...
				// Ensure that all nodes to the left of the split are strictly less than the split value
				while (mid < end && nodes[mid].position[axis] < tree[index].split) mid++;
				while (mid > start && nodes[mid-1].position[axis] >= tree[index].split) mid--;
		...
		void Remove (GraphNode point, int index, int depth = 0) {
			// Move down in the tree until the leaf node is found that this point is inside of
			while (tree[index].data == null) {
				index = 2 * index + (point.position[tree[index].splitAxis] < tree[index].split ? 0 : 1);
				depth++;
			...
			if (idx == -1) throw new System.ArgumentException("The node is not in the lookup tree. Has it been moved?");
