...
{	
	...
	public void Play( AnimationClip anim, float speed = 1, bool startOnRandomFrame = false) 
	{
		if ( anim == null )
			return;
		...
		if (startOnRandomFrame) m_animator.Update(Random.Range(0f,anim.length));
	}		 	 
	...
	void Awake()
	{
		...
		if (m_defaultAnim) Play(m_defaultAnim);
	}
